...
def sort_sources(sources):
    ...
    steven_black_positions = [
        x for x, y in enumerate(result) if "nholuongut" in y.lower()
    ]
...
def write_opening_header(final_file, **header_params):
    ...
    if header_params["extensions"]:
        if no_unified_hosts:
            if len(header_params["extensions"]) > 1:
                write_data(
                    final_file,
                    "# Title: nholuongut/hosts extensions {0} and {1} \n#\n".format(
                ...
                write_data(
                    final_file,
                    "# Title: nholuongut/hosts extension {0}\n#\n".format(
            ...
            if len(header_params["extensions"]) > 1:
                write_data(
                    final_file,
                    "# Title: nholuongut/hosts with the {0} and {1} extensions\n#\n".format(
                        ", ".join(header_params["extensions"][:-1]),
                        header_params["extensions"][-1],
                    ),
                )
                ...
                write_data(
                    final_file,
                    "# Title: nholuongut/hosts with the {0} extension\n#\n".format(
                        ", ".join(header_params["extensions"])
                    ),
                )
    else:
        write_data(final_file, "# Title: nholuongut/hosts\n#\n")
    ...
    write_data(
        ...
        "https://raw.githubusercontent.com/nholuongut/hosts/master/"
        + path_join_robust(header_params["outputsubfolder"], "").replace("\\", "/")
        + "hosts\n",
    )
    write_data(
        final_file, "# Project home page: https://github.com/nholuongut/hosts\n"
    )
    write_data(
        final_file,
        "# Project releases: https://github.com/nholuongut/hosts/releases\n#\n",
    )
    write_data(
        final_file,
        "# ===============================================================\n",
